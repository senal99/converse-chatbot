...
const Chatbot: React.FC = () => {
  ...
  const formatMessageText = (text: string) => {
      ...
      for (let i = 0; i < filteredText.length; i++) { // It'll loop the whole thing
        ...
        if (/^\d+\.\s/.test(currentItem)) {
          ...
          if (periodPositions.length >= 2) {
            ...
            filteredText[i] =
              currentItem.slice(0, indexOfSecondPeriod + 1) + '<br /> <br />' + currentItem.slice(indexOfSecondPeriod + 1);
          }
        }
      }
  ...
  return (
    // <div  className={` mt-6 xl:mt-0 w-full sm:w-[400px] border ${colors.normalText} border-gray-200 pt-2 ${colors.backgroundBox} rounded-lg overflow-hidden`}>
       <div  className={` ${colors.normalText} border sm:w-[400px] p-2 border-gray-200 ${colors.backgroundBox} rounded-lg overflow-hidden`}>
      <div id="message-container"  className="h-96 pl-6 pr-6 pb-1 rounded-md overflow-auto">
        <div className=''>
          {messages.length ?
            messages.map((message, index) => (
              <div key={index} className={`flex pt-4 ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                {!message.isUser ?
                  ...
                  : ""}
                <div
                  className={`w-10/12 p-4 mb-2 ${message.isUser ? `rounded-tl-xl break-words rounded-br-xl rounded-bl-xl ${colors.primary} text-left text-white` : `rounded-tr-xl rounded-br-xl rounded-bl-xl ${colors.dropDownBg} text-left`}`}
                ...
                {message.isUser ?
                  <Avatar className='ml-2'>
                    ...
                    <AvatarFallback className={`${colors.primary}`}><User2 /></AvatarFallback>
                  </Avatar>
                  : ""}
              </div>
            ))
            ...
            <div className='pt-6 text-center'>
              <div className='text-3xl pb-4'>Welcome to <span className={`font-bold ${colors.colorHeading}`}>Converse</span> </div>
      ...
      <div className="flex w-full p-2 rounded-lg">
        ...
        <Button className={`m-1 ml-1 ${colors.noButton}`} onClick={handleClearMessages} disabled={loading}><Eraser /></Button>
        <Button className={`m-1 ${colors.okButton}`} type='submit' onClick={handleSendMessage} disabled={loading}>Ask</Button>
      </div>
    </div>
  );
};
